 ...
 *      Если явно не указан, то проверяется начальные символы value на соответствие заложенным операторам ~,>,<,=,!,[,],>=,<=,!=,!~,(),!()
 ...
 * @property {Object} locate настройка для позиционирования в массиве данных на указанный элемент. Если происходит позиционирование, то будет отдан кусок данный его содержащий невзирая на range.
 ...
 * @property {boolean} treeMode.filterByHid признак, производить ли фильтрации. по узлй или нет
 ...
 * @property {Array<Object|Array>|Object} data возвращаемые данные
 ...
 *      array - массив с длинной в количество отдаваемых колонок
 *      object - объект со свойствами - колонками ответа
 ...
 * @property {boolean} [returnFirst] признак, по которому возвращается в data только первый найденный элемент выборки или вся выборка массивом
 */
 ...
 * @property {string} name имя провайдера. Повторяет свойство узла экземпляра провайдера из конфигурационного файла приложения
 * @property {string} type тип источника данных, по которому потом инстанцируются экземпляры в модулях, где они имплементированы (например: db-postgres)
 * @property {string} connectType способ соединения с источником
 *      user - когда каждый запрос на соединение формируется отдельно
 *      pool - когда создается общий пул соединений при первом запросе на соединение под параметрами именного этого запроса
 *      poolPerUser - создаются пулы соединений под каждого пользователя источника данных, когда под ними было запрошено соединение
 * @property {Object} connectPoolConfig дополнительные настройки пула соединений с источником. Формат от типа источника
 * @property {'session'|'config'} credentialsSource откуда брать параметры пользователя пароля и т.п. для соединения (или формирования пула)
 *      session - из сессии пользователя веб-сервера
...
export class NFProvider {
    ...
    constructor(config) {
        ...
        if (tunnelConfig && tunnelConfig.host) {
            ...
            import('tunnel-ssh').then((sshTunnel) => {
                tunnelConfig.keepAlive = true;
                const srv = sshTunnel.default(tunnelConfig, (err, server) => {
                    if (err) {
