...
class Connect {
    ...
    async query(queryString, params = {}, options = {}, control = {}) {
        ...
        return this._provider.query(this._connect, queryString, params, _options, control);
    }
    ...
    try {
        ...
        if (Array.isArray(queryString)) {
            ...
            for (let i = 0, c = queryString.length; i < c; i++) {
                result.push(await connect.query(queryString[i], params, queryOptions, control));
            }
        } else {
            result = await connect.query(queryString, params, queryOptions, control);
        }
        await connect.commit();
    } catch (e) {
        if(connect) {
